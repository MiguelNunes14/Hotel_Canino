...
import bookingListExporterFields from '@/modules/booking/booking-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (
          !bookingListExporterFields ||
          !bookingListExporterFields.length
        ) {
          throw new Error(
            'bookingListExporterFields is required',
          );
        }
