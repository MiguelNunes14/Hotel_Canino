...
import petListExporterFields from '@/modules/pet/pet-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (
          !petListExporterFields ||
          !petListExporterFields.length
        ) {
          throw new Error(
            'petListExporterFields is required',
          );
        }
